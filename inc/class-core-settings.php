...
class Iconic_WAS_Core_Settings {
	...
	private function __construct() {
		...
		if ( defined( 'ICONIC_WAS_FILE' ) ) {
			add_filter( 'plugin_action_links_' . plugin_basename( ICONIC_WAS_FILE ), array( __CLASS__, 'plugin_action_links' ) );
		}
	}
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'iconic-was-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'iconic-was' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_url( self::get_docs_url() . $link['href'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=getting-started-links&utm_content=iconic-was" target="_blank"><?php echo esc_html( $link['title'] ); ?></a>
				</li>
			<?php } ?>
		</ol>
		<?php
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'iconic-was' ),
		);
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_WAS_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'iconic-was' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'license_field',
						'title'    => __( 'License Key', 'iconic-was' ),
						'subtitle' => __( 'Activate your license to get the latest updates and support.', 'iconic-was' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'iconic-was' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'iconic-was' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'iconic-was' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'iconic-was' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'iconic-was' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_WAS_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Iconic_WAS_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'iconic-was' ); ?></h3>
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['link'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=cross-sell&utm_content=iconic-was" target="_blank"><?php echo wp_kses_post( $cross_sell['title']['rendered'] ); ?></a></h4>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', 'iconic-was' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'iconic-was' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=support-link&utm_content=iconic-was', esc_html__( 'Submit Ticket', 'iconic-was' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=documentation-link&utm_content=iconic-was" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), esc_html__( 'Read Documentation', 'iconic-was' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://my.iconicwp.com/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=account-link&utm_content=iconic-was', esc_html__( 'Manage Your Account', 'iconic-was' ) );
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'iconic-was' ) . '">' . esc_html__( 'Settings', 'iconic-was' ) . '</a>',
		);
